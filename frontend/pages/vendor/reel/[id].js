...
import { getVendorReel } from "@/services/vendor";
...
const UpdateReel = ({ reel }) => {
  ...
  const {
    ...
    defaultValues: {
      ...
      isActive: {
        label: reel.isActive ? t("Active") : t("Inactive"),
        value: reel.isActive,
      },
    },
  });
  ...
  return (
    <>
      <Layout seoData={{ pageTitle: t("Update Reel - Noonmar") }}>
        <div className="main_content listingContainer">
          <div className="dash-titles">
            <h2 className="dash-h2">{t("Update Reel")}</h2>
          </div>
...
export async function getServerSideProps(context) {
  ...
  const { reel } = await getVendorReel(id);

  if (!reel) {
    return {
      redirect: {
        permanent: false,
        destination: "/vendor/reels",
      },
    };
  }
