...
const ForgotPassword = ({ image }) => {
  ...
  useEffect(() => {
    if (response) {
      if (response.status) {
        ...
        router.push(
          {
            pathname: "/customer/forgot-password-verification",
            query: {
              id: response.id,
  ...
  const onsubmit = (data) => {
    ...
    request("POST", "v1/customer/forgot-password", { emailOrPhone });
  };
...
export async function getServerSideProps(context) {
  const image = await getSystemImage("customer-forgot-password");
