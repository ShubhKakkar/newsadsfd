...
exports.getAllByCountry = async (req, res, next) => {
  ...
  const getCategoriesHandler = async (category, level) => {
    const project = {};

    // if (level < 1) {
    //   project.slug = "$langData.slug";
    // }
    ...
    for (let i = 0; i < productCategories.length; i++) {
      await getCategoriesHandler(productCategories[i], level);
    }
  };
  ...
  for (let i = 0; i < productCategories.length; i++) {
    await getCategoriesHandler(productCategories[i], 0);
  }
