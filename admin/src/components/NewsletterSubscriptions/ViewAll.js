...
const searchQueryHandler = (
  ...
  return `newsletter/all?page=${page}&per_page=${per_page}&sortBy=${sortBy}&order=${order}&isActive=${isActive}&email=${email}&dateFrom=${dateFrom}&dateTo=${dateTo}`;
};
...
const ViewAll = () => {
  ...
  useEffect(() => {
    ...
    document.title = "Newsletter Subscriptions - Noonmar";
  }, [records_per_page]);

  useEffect(() => {
    if (response) {
      setRecords(response.newsletters);
  ...
  const deleteHandler = (id) => {
    MySwal.fire({
      ...
      if (result.isConfirmed) {
        requestDelete("DELETE", "newsletter", { id });
  ...
  const changeStatusHandler = (id) => {
    ...
    MySwal.fire({
      ...
      if (result.isConfirmed) {
        requestChangeStatus("PUT", "newsletter/status", {
          id,
          status,
        });
      } else if (result.isDismissed) {
