...
const searchQueryHandler = (
  ...
  return `report-reason/all?page=${page}&per_page=${per_page}&sortBy=${sortBy}&order=${order}&isActive=${isActive}&title=${title}&dateFrom=${dateFrom}&dateTo=${dateTo}`;
};
...
const ViewAll = () => {
  ...
  useEffect(() => {
    ...
    document.title = "Report Reasons- Noonmar";
  }, [records_per_page]);
  ...
  const fetchMoreData = ({ selected }) => {
    setSeekers([]);
    const { isActive, title, dateFrom, dateTo } = getValues();
  ...
  const deleteHandler = (id) => {
    MySwal.fire({
      ...
      if (result.isConfirmed) {
        requestDelete("DELETE", "report-reason", { id });
  ...
  const changeStatusHandler = (id) => {
    ...
    MySwal.fire({
      ...
      if (result.isConfirmed) {
        requestChangeStatus("PUT", "report-reason/status", {
          id,
          status,
        });
      } else if (result.isDismissed) {
              ...
              <div className="card card-custom card-stretch card-shadowless">
                <div className="card-header">
                  ...
                  <div className="card-toolbar">
                    ...
                    {((roleId === 2 && !!permission["8_2"]) || roleId == 1) && (
                      <Link to="/report-reason/add" className="btn btn-primary">
                        Add New Reason
                      </Link>
                    )}
                  </div>
