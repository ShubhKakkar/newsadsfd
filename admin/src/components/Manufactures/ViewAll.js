...
const searchQueryHandler = (
  ...
  return `manufacture/all?page=${page}&per_page=${per_page}&sortBy=${sortBy}&order=${order}&isActive=${isActive}&country=${country}&name=${name}&dateFrom=${dateFrom}&dateTo=${dateTo}`;
};
...
const ViewAll = () => {
  ...
  useEffect(() => {
    ...
    document.title = "Manufactures - Noonmar";
  }, [records_per_page]);
  ...
  useEffect(() => {
    if (responseDelete) {
      ...
      let newSeeker = [...manufactures];
      newSeeker = newSeeker.filter((seeker) => seeker._id != id);
      setManufactures(newSeeker);
  ...
  const deleteHandler = (id) => {
    MySwal.fire({
      ...
      if (result.isConfirmed) {
        requestDelete("DELETE", "manufacture", { id });
  ...
  const changeStatusHandler = (id) => {
    const user = manufactures.find((seeker) => seeker._id == id);
    ...
    MySwal.fire({
      ...
      if (result.isConfirmed) {
        requestChangeStatus("PUT", "manufacture/status", {
          id,
          status,
        });
      } else if (result.isDismissed) {
                ...
                <div className="card-header">
                  ...
                  <div className="card-toolbar">
                    ...
                    {((roleId === 2 && !!permission["9_2"]) || roleId == 1) && (
                      <Link to="/manufacture/add" className="btn btn-primary">
                        Add New Manufacture
                      </Link>
                    )}
                  </div>
                  ...
                  <div className="dataTables_wrapper ">
                    ...
                    {perPage !== 0 && (
                      <Pagination
                        ...
                        currentDocLength={manufactures.length}
                      />
                    )}
                  </div>
                </div>
