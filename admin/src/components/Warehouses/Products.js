...
const searchQueryHandler = (
  ...
  return `warehouse/products?page=${page}&per_page=${per_page}&sortBy=${sortBy}&order=${order}&isActive=${isActive}&name=${name}&vendor=${vendor}&masterCategoryId=${masterCategoryId}&dateFrom=${dateFrom}&dateTo=${dateTo}&brandId=${brandId}&customId=${customId}&price=${price}&warehouseId=${warehouseId}&isSelected=${isSelected}&barCode=${barCode}`;
};
...
const ViewAll = (props) => {
  ...
  const selectHandler = (value, id) => {
    if (id === "all") {
      if (value) {
        //select all
        products.forEach((seeker) => {
          setValueSelected(seeker._id, true);
        });
        setValueSelected("selectedCount", products.length);
      } else {
        //unselect all
        products.forEach((seeker) => {
          setValueSelected(seeker._id, false);
        });
        setValueSelected("selectedCount", 0);
      }
    } else {
      const oldSelectedCount = watchSelected("selectedCount");

      if (value) {
        setValueSelected("selectedCount", oldSelectedCount + 1);

        if (products.length === oldSelectedCount + 1) {
          //select all
          setValueSelected("all", true);
        }
      } else {
  ...
  const onSubmit = (data) => {
    ...
    for (let key in data) {
      if (!["all", "selectedCount"].includes(key)) {
        const value = data[key];

        if (value) {
          selectedIds.push(key);
        } else {
          notSelectedIds.push(key);
        }
      }
    }
  ...
  const deleteHandler = (id) => {
    MySwal.fire({
      title: "Are you sure?",
      text: "You want to delete this product from warehouse?",
      icon: "warning",
