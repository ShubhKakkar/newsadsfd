...
const searchQueryHandler = (
  ...
  return `warehouse/all?page=${page}&per_page=${per_page}&sortBy=${sortBy}&order=${order}&isActive=${isActive}&name=${name}&vendor=${vendor}&dateFrom=${dateFrom}&dateTo=${dateTo}&role=customer`;
};
...
const ViewAll = () => {
  ...
  const deleteHandler = (id) => {
    MySwal.fire({
      ...
      if (result.isConfirmed) {
        requestDelete("DELETE", "warehouse", { id });
  ...
  const changeStatusHandler = (id) => {
    ...
    MySwal.fire({
      ...
      if (result.isConfirmed) {
        requestChangeStatus("PUT", "warehouse/status", {
          id,
          status,
        });
      } else if (result.isDismissed) {
