...
const ViewOne = (props) => {
  ...
  useEffect(() => {
    if (records_per_page) {
      requestData(
        "GET",
        `warehouse/${warehouseId}?per_page=${records_per_page}`
      );

      document.title = "View Warehouse - Noonmar";
  ...
  useEffect(() => {
    if (responseData) {
      ...
      // setTotalDocuments(responseData.products.totalDocuments);

      setUserData([
        { title: "Warehouse Name/Label", value: name },
        // {
        //   title: "Vendor",
        ...
        {
          title: "Zipcode",
          value: zipCode?.length > 0 ? zipCode : "-",
        },
        {
          title: "Street",
          value: street,
  ...
  const fetchMoreData = ({ selected }) => {
    ...
    requestProducts(
      ...
      `warehouse/products?page=${selected + 1}&per_page=${perPage}&sortBy=${
        ...
        new Date()
      )}&warehouseId=${warehouseId}&isSelected=true`
    );
  };
            ...
            <div className="card-header card-header-tabs-line">
              <div className="card-toolbar">
                <ul
                  ...
                  role="tablist"
                >
                  {/* ["Warehouse Information", "Products"] */}
                  {["Warehouse Information"].map((data, index) => (
                    <SubTab
                      key={index}
                      name={data}
                      index={index}
