...
const Edit = (props) => {
  ...
  useEffect(() => {
    if (id) {
      document.title = "Edit Country Group - Noonmar";
  ...
  useEffect(() => {
    if (responseVendor) 
      ...
      {
      if (responseVendor.status && responseVendor.countries) {
        requestMembers("GET", `group/country/${id}`);
        setAllMembers(
          responseVendor.countries.map((data) => ({
            label: data.label,
            value: data._id,
          }))
        );
  ...
  useEffect(() => {
    if (response) {
      ...
      toast.success("Country Group updated successfully.");
  ...
  const onSubmit = (data) => {
    ...
    request("PUT", "group/update/country", {
      name,
      members,
      id,
    });
    ...
    >
      <Breadcrumb
        ...
        links={[
          { to: "/", name: "Dashboard" },
          {
            to: { pathname: "/group/countries" /*backPageNum: page */ },
            name: "Back To Country Group",
          },
        ]}
      />
