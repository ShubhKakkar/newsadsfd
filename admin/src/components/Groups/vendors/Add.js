...
const Add = () => {
  ...
  useEffect(() => {
    ...
    requestMembers("GET", `vendor/get-all-vendor`);
  }, []);

  useEffect(() => {
    if (responseMembers) {
      if (responseMembers.status && responseMembers.vendor) {
        setAllMember(
          responseMembers.vendor.map((data) => ({
            label: data.label,
            value: data._id,
          }))
        );
  ...
  useEffect(() => {
    if (response) {
      toast.success("Vendor Group added successfully.");
  ...
  return (
    ...
    >
      <Breadcrumb
        ...
        links={[
          { to: "/", name: "Dashboard" },
          { to: "/group/vendors", name: "Back To Vendor Group" },
        ]}
      />
