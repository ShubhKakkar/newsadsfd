...
const Edit = (props) => {
  ...
  useEffect(() => {
    if (id) {
      document.title = "Edit Vendor Group - Noonmar";
  ...
  useEffect(() => {
    if (responseVendor) {
      if (responseVendor.status && responseVendor.vendor) {
        requestMembers("GET", `group/vendor/${id}`);
        setAllMembers(
          responseVendor.vendor.map((data) => ({
            label: data.label,
            value: data._id,
          }))
        );
  ...
  const onSubmit = (data) => {
    ...
    request("PUT", "group/update/vendor", {
      name,
      members,
      id,
    });
    ...
    >
      <Breadcrumb
        ...
        links={[
          { to: "/", name: "Dashboard" },
          {
            to: { pathname: "/group/vendors" /*backPageNum: page */ },
            name: "Back To Vendor Group",
          },
        ]}
      />
