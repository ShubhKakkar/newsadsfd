...
const Edit = (props) => {
  ...
  useEffect(() => {
    if (id) {
      document.title = "Edit Supplier Group - Noonmar";
  ...
  useEffect(() => {
    if (responseVendor) 
      ...
      {
      if (responseVendor.status && responseVendor.manufactures) {
        requestMembers("GET", `group/supplier/${id}`);
        setAllMembers(
          responseVendor.manufactures.map((data) => ({
            label: data.label,
            value: data._id,
          }))
        );
  ...
  const onSubmit = (data) => {
    ...
    request("PUT", "group/update/supplier", {
      name,
      members,
      id,
    });
    ...
    >
      <Breadcrumb
        ...
        links={[
          { to: "/", name: "Dashboard" },
          {
            to: { pathname: "/group/suppliers" /*backPageNum: page */ },
            name: "Back To Supplier Group",
          },
        ]}
      />
