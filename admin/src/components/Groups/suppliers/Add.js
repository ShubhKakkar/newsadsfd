...
const Add = () => {
  ...
  useEffect(() => {
    ...
    requestMembers("GET", `manufacture/get-all-supplier-manufactures`);
  }, []);

  useEffect(() => {
    if (responseMembers) {
      if (responseMembers.status && responseMembers.manufactures) {
        setAllMember(
          responseMembers.manufactures.map((data) => ({
            label: data.label,
            value: data._id,
          }))
        );
  ...
  useEffect(() => {
    if (response) {
      toast.success("Supplier Group added successfully.");
      history.push("/group/suppliers");
    }
  }, [response]);
  ...
  const onSubmit = (data) => {
    ...
    request("POST", "group/add/supplier", {
      name,
      members,
    });
  };
  ...
  return (
    ...
    >
      <Breadcrumb
        ...
        links={[
          { to: "/", name: "Dashboard" },
          { to: "/group/suppliers", name: "Back To Supplier Group" },
        ]}
      />
