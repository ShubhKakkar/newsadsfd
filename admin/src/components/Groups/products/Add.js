...
const Add = () => {
  ...
  useEffect(() => {
    ...
    requestMembers("GET", `product/get-all-products`);
  }, []);

  useEffect(() => {
    if (responseMembers) {
      if (responseMembers.status && responseMembers.products) {
        setAllMember(
          responseMembers.products.map((data) => ({
            label: data.label,
            value: data._id,
          }))
        );
  ...
  const onSubmit = (data) => {
    ...
    request("POST", "group/add/product", {
      name,
      members,
    });
  };
  ...
  return (
    ...
    >
      <Breadcrumb
        ...
        links={[
          { to: "/", name: "Dashboard" },
          { to: "/group/products", name: "Back To Product Group" },
        ]}
      />
