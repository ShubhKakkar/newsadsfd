...
const Edit = (props) => {
  ...
  useEffect(() => {
    if (id) {
      document.title = "Edit Product Group - Noonmar";
  ...
  useEffect(() => {
    if (responseVendor) {
      if (responseVendor.status && responseVendor.products) {
        requestMembers("GET", `group/product/${id}`);
        setAllMembers(
          responseVendor.products.map((data) => ({
            label: data.label,
            value: data._id,
          }))
        );
  ...
  const onSubmit = (data) => {
    ...
    request("PUT", "group/update/product", {
      name,
      members,
      id,
    });
