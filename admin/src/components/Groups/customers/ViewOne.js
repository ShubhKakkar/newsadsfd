...
const ViewOne = (props) => {
  ...
  const { response: responseGroupData, request: requestGroupData } = useRequest();

  const { date_format } = useSelector((state) => state.setting);

  useEffect(() => {
    requestGroupData("GET", `group/customer/${recordId}`);
  ...
  return (
    ...
    >
      <Breadcrumb
        ...
        links={[
          { to: "/", name: "Dashboard" },
          { to: "/group/customers", name: "Back To Groups" },
        ]}
      />
