...
import { getEntity, getEntityTextONE, getEntityTextTWO } from "../../util/fn";
...
const ViewOne = (props) => {
  const { id: recordId, entityType } = props.match.params;
  ...
  useEffect(() => {
    requestGroupData("GET", `group/${entityType}/${recordId}`);
  ...
  useEffect(() => {
    if (responseGroupData) {
      const { name, members, createdAt, isActive } = responseGroupData.data;
