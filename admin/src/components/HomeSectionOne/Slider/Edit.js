...
const Edit = (props) => {
  ...
  useEffect(() => {
    requestGetSection("GET", `master/home/section-one/slider/${id}`);

    document.title = "Section 1 (Edit) - Noonmar";
  }, []);

  useEffect(() => {
    if (responseGetSection) {
      const { sliderData } = responseGetSection;

      if (!sliderData) {
        history.push("/section-one/sliders");
        return;
      }
      const { langData, link } = sliderData;
  ...
  const onSubmitNew = (data) => {
    ...
    request("PUT", "master/home/section-one/slider", formData);
  };
