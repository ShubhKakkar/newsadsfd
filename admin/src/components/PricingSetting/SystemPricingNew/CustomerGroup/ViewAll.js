...
const ViewCustomerGroups = (props) => {
  ...
  const searchQueryHandler = (
    ...
    return `pricing-new/system/customer-groups?page=${page}&per_page=${per_page}&sortBy=${sortBy}&order=${order}&isActive=${isActive}&name=${name}&type=${type}&dateFrom=${dateFrom}&dateTo=${dateTo}&parentType=default`;
  };
  ...
  useEffect(() => {
    ...
    document.title =
      type == "customer"
        ? "Customer Group Pricing- Noonmar"
        : "Product Group Pricing- Noonmar";
  ...
  const deleteHandler = (id) => {
    MySwal.fire({
      ...
      if (result.isConfirmed) {
        requestDelete("DELETE", "pricing-new/system/group", { id });
  ...
  const changeStatusHandler = (id) => {
    ...
    MySwal.fire({
      ...
      if (result.isConfirmed) {
        requestChangeStatus("PUT", "pricing-new/system/group/status", {
          id,
          status,
        });
      } else if (result.isDismissed) {
                ...
                <div className="card-header">
                  ...
                  <div className="card-toolbar">
                    ...
                    {((roleId === 2 && !!permission["7_2"]) || roleId == 1) && (
                      <Link
                        to={`/system-pricing/${type}/add`}
                        className="btn btn-primary"
                      >
                        Add {type == "customer" ? "Customer" : "Product"} Group
                        Pricing
                      </Link>
                    )}
                  </div>
