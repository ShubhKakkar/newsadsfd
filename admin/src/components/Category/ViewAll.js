...
const titleSingular = "Category";
const titlePlural = "Categories";
...
const searchQueryHandler = (
  ...
  return `${apiName}/all?page=${page}&per_page=${per_page}&sortBy=${sortBy}&order=${order}&isActive=${isActive}&name=${name}&dateFrom=${dateFrom}&dateTo=${dateTo}&parentId=${recordId}`;
};
...
const ViewAll = (props) => {
  ...
  const { request: requestGetOne, response: responseGetOne } = useRequest();

  useEffect(() => {
    if (recordId) {
      requestGetOne("GET", `category/${recordId}`);
    }
  }, [recordId]);

  useEffect(() => {
    if (responseGetOne) {
      if (responseGetOne.category) {
        setParentCategory({
          isSubCategory: false,
          id: responseGetOne.category.parentId,
        });
      } else if (responseGetOne.data) {
        const id = responseGetOne.data?.data?.productCategoryId;
        if (id) {
          setParentCategory({
            isSubCategory: true,
            id,
        ...
        requestGetOne("GET", `product-sub-category/${recordId}`);
      }
    }
  }, [responseGetOne]);
  ...
  }, [records_per_page, recordId]);

  useEffect(() => {
    if (response) {
      setDataList(response.categories);
    ...
    >
      <Breadcrumb
        ...
        links={[
          { to: "/", name: "Dashboard" },
          {
            to: parentCategory.isSubCategory
              ? `/product/sub-categories/${parentCategory.id}`
              : `/product/sub-categories-multi/${parentCategory.id}`,
                ...
                <div className="card-header">
                  ...
                  <div className="card-toolbar">
                    ...
                    {((roleId === 2 && !!permission["12_191"]) ||
                      roleId == 1) && (
                      <Link
                        to={`/product/sub-categories-multi/add/${recordId}`}
                  ...
                  <div className="dataTables_wrapper ">
                    <Table
                      ...
                      links={[
                        ...
                        {
                          isLink: true, 
                          to: "/product/sub-categories-multi/edit",
                          name: "Edit",
                          extraData: true,
                          key: "12_193",
                        },
                        {
                          isLink: true,
                          to: "/product/sub-categories-multi",
                          name: "Read",
