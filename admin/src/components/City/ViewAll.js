...
const searchQueryHandler = (
  ...
  return `city/all?page=${page}&per_page=${per_page}&sortBy=${sortBy}&order=${order}&isActive=${isActive}&name=${name}&dateFrom=${dateFrom}&dateTo=${dateTo}&parentId=${parentId}`;
};
...
const ViewAll = (props) => {
  ...
  const [grandFatherId, setGrandFatherId] = useState(null);
  ...
  useEffect(() => {
    if (response) {
      ...
      setHideAddButton(response.hideAddButton);
      setGrandFatherId(response.grandFatherId);
    }
  }, [response]);
  ...
  useEffect(() => {
    if (responseChangeStatus) {
      ...
      const indexToChange = newCities.findIndex((city) => city._id == id);
      newCities[indexToChange].isActive = newStatus;
  ...
  useEffect(() => {
    if (responseDelete) {
      ...
      newCities = newCities.filter((city) => city._id != id);
      setCities(newCities);
  ...
  const changeStatusHandler = (id) => {
    const user = cities.find((city) => city._id == id);
    ...
    MySwal.fire({
      ...
      if (result.isConfirmed) {
        requestChangeStatus("PUT", "city/status", {
          id,
          status,
        });
      } else if (result.isDismissed) {
  ...
  const breadcrumbLinks = [{ to: "/", name: "Dashboard" }];
  const links = [];
  ...
  if (grandFatherId) {
    breadcrumbLinks.push({
      to: `/cities/${grandFatherId}`,
      name: "Back To Cities",
    });
  } else {
    breadcrumbLinks.push({
      to: `/countries`,
      name: "Back To Countries",
    });
  }
  ...
  return (
    <div
      ...
      id="kt_content"
    >
      <Breadcrumb title="Cities" links={breadcrumbLinks} />
              ...
              <div className="card card-custom card-stretch card-shadowless">
                <div className="card-header">
                  ...
                  <div className="card-toolbar">
                    ...
                    {((roleId === 2 && !!permission["12_81"]) ||
                      roleId == 1) && (
                      <Link
                        to={`/city/${parentId}/add`}
                  ...
                  <div className="dataTables_wrapper ">
                    ...
                    {perPage !== 0 && (
                      <Pagination
                        ...
                        currentDocLength={cities.length}
                      />
                    )}
                  </div>
                </div>
