...
const searchQueryHandler = (
  ...
  return `subscription-offer/all?page=${page}&per_page=${per_page}&sortBy=${sortBy}&order=${order}&isActive=${isActive}&tenure=${tenure}&dateFrom=${dateFrom}&dateTo=${dateTo}`;
};
...
const ViewAll = () => {
  ...
  useEffect(() => {
    ...
    document.title = "Subscription Offers- Noonmar";
  }, [records_per_page]);
  ...
  useEffect(() => {
    if (responseDelete) {
      ...
      toast.success("Subscription offer has been deleted successfully.");
    }
  }, [responseDelete]);

  const fetchMoreData = ({ selected }) => {
    setSeekers([]);
    const { isActive, tenure, type, price, dateFrom, dateTo } = getValues();
  ...
  const deleteHandler = (id) => {
    MySwal.fire({
      title: "Are you sure?",
      text: "You want to delete this subscription offer?",
      icon: "warning",
      ...
      if (result.isConfirmed) {
        requestDelete("DELETE", "subscription-offer", { id });
  ...
  const onSearchHandler = (data) => {
    const { isActive, tenure, dateFrom, dateTo } = getValues();
  ...
  const changeStatusHandler = (id) => {
    ...
    MySwal.fire({
      ...
      if (result.isConfirmed) {
        requestChangeStatus("PUT", "subscription-offer/status", {
          id,
          status,
        });
      } else if (result.isDismissed) {
              ...
              <div className="card card-custom card-stretch card-shadowless">
                <div className="card-header">
                  ...
                  <div className="card-toolbar">
                    ...
                    {((roleId === 2 && !!permission["5_21"]) ||
                      roleId == 1) && (
                      <Link
                        to="/subscription-offer/add"
                        className="btn btn-primary"
                      >
                        Add New Subscription Offer
                      </Link>
                    )}
                  </div>
                </div>
